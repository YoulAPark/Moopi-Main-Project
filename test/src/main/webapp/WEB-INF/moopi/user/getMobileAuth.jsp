...
// [완료] by.율아
		...
		// 들어온 페이지 값 지정 : addUser - 회원가입 / findId - 아이디찾기 / updatePwd - 비밀번호 찾기 및 수정
		var href = document.location.search;	
	
		function fncKey() {
		
			// userId : 유저아이디 / AuthNum : 인증번호 / phone : 본인 모바일번호
			...
			if(AuthNum == key){				
				...
				$(function() {	
						...
						// 회원가입 경로로 접속했을경우
						// * opener.document.getElementById : 자식창에서 부모창 제어할때 사용
						} else if (href == "?addUser") {		
							opener.document.getElementById("pInput").value = $("#pnNum").val()
							window.close();
							return;
								
						// 비밀번호찾기 및 수정 경로로 접속했을경우
...
// Cool SMS API
		...
		function fncAuth(){
			...
			// 입력값이 null이거나 11자리가 아닐 경우 경고창 띄우기
			if(phone == null || phone.length != 11){			
				swal("올바른 형식의 모바일번호가 아닙니다.","","warning");
				return;
			}
